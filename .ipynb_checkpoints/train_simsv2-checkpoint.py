...
def get_appropriate_dataset(data):
    ...
    if isinstance(data, dict):
        ...
        features = []
        # simsv2 text_bert already includes fixed-size text features; use its length directly
        target_len = text_bert.shape[-1]

        for idx in range(len(labels)):
            bert_block = text_bert[idx]
            # For simsv2 these are already length `target_len`; keep as-is
            input_ids = bert_block[0].astype(np.float32).tolist()
            input_mask = bert_block[1].astype(np.float32).tolist()
            segment_ids = bert_block[2].astype(np.float32).tolist()
    ...
    # simsv2 provides BERT-processed representations; keep them as float tensors to avoid
    # unnecessary DeBERTa encoding downstream.
    input_dtype = torch.float if isinstance(data, dict) else torch.long
    all_input_ids = torch.tensor(np.array([f.input_ids for f in features]), dtype=input_dtype)
