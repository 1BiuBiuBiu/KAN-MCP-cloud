...
class Transformer(nn.Module):
    def __init__(self, embed_dim, num_heads, layers, latent_dim, attn_dropout=0.1, relu_dropout=0.1, res_dropout=0.1,
                 embed_dropout=0.1, attn_mask=False, use_cls_token=False):
        super().__init__()
        self.use_cls_token = use_cls_token

        self.loss_fct = nn.MSELoss()

        # Encode with an optional cls token, then project into a latent space.
...
class TransformerEncoder(nn.Module):
    ...
    def __init__(self, embed_dim, num_heads, layers, attn_dropout=0.0, relu_dropout=0.0, res_dropout=0.0,
                 embed_dropout=0.0, attn_mask=False, use_cls_token=True):
        super().__init__()
    ...
    def forward(self, x_in, x_in_k=None, x_in_v=None):
        ...
        if self.use_cls_token:
            ...
            if x_k is not None and x_v is not None:
                # Mirror cls so cross-attention keeps a pooled position as well.
                x_k = torch.cat([cls_tokens, x_k], dim=0)
                x_v = torch.cat([cls_tokens, x_v], dim=0)

        # embed tokens and positions
        x = self.embed_scale * x
        if self.embed_positions is not None:
            x += self.embed_positions(x.transpose(0, 1)[:, :, 0]).transpose(0, 1)  # Add positional embedding
        ...
        if x_k is not None and x_v is not None:
            ...
            if self.embed_positions is not None:
                x_k += self.embed_positions(x_k.transpose(0, 1)[:, :, 0]).transpose(0, 1)  # Add positional embedding
...
if __name__ == '__main__':
    encoder = TransformerEncoder(300, 4, 2, use_cls_token=True)
